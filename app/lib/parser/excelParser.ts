 ...
 * @param file
 * @param options
 * @returns Promise<{sheet: string, content: string}[]>
 */
export async function excelToCsv(
  file: File,
  options: ExcelParserOptions = {}
): Promise<{ sheet: string; content: string }[]> {
    ...
    reader.onload = (e) => {
      try {
        ...
        const results = sheets.map(({ name, worksheet }) => {
          ...
          const columns = Array.from({ length: maxColumns - 64 }, (_, i) =>
          ...
          if (format === 'markdown') {
            // Markdown格式
            content +=
              '| ' + columns.map((col) => jsonData[0][col]).join(' | ') + ' |\n'
