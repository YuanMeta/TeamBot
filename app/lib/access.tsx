...
export const AccessContext = createContext<{ access: string[] }>({ access: [] })
...
export function AccessProvider({ children }: { children: React.ReactNode }) {
  ...
  useLayoutEffect(() => {
    trpc.common.getUserAccess.query().then((res) => {
      setState({ access: res, ready: true })
    })
  }, [])
  if (!state().ready) return null
  return (
    <AccessContext value={{ access: state().access }}>{children}</AccessContext>
  )
}
